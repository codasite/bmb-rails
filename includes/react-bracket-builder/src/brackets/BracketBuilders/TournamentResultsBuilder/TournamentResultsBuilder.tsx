...
const CustomCheckbox = (props: any) => {
	...
	const styles = baseStyles.concat(checked ? checkedStyles : uncheckedStyles).join(' ')

	return (
		<input type='checkbox' id={id} className={styles} checked={checked} onChange={onChange} style={{ backgroundImage: checked ? `url(${checkIcon})` : 'none' }} />
	)
}
		...
		<BracketMetaContext.Provider value={{ title: bracketTitle, date: bracketDate }}>
			<DarkModeContext.Provider value={darkMode}>
				<div className={`wpbb-reset tw-uppercase tw-bg-no-repeat tw-bg-top tw-bg-cover${darkMode ? ' tw-dark' : ''}`} style={{ 'backgroundImage': `url(${darkMode ? darkBracketBg : lightBracketBg})` }}>
					{
						matchTree &&
						<div className={`tw-flex tw-flex-col tw-items-center tw-max-w-screen-lg tw-m-auto tw-pb-[57px]`}>
							...
							<div className={`tw-flex tw-flex-col tw-gap-24${!complete ? ' tw-max-w-[470px] tw-w-full' : ''}`}>
								...
								<div className='tw-flex tw-gap-20 tw-items-center tw-self-center tw-hidden'>
									...
									/>
									<label htmlFor='notify-participants-check' className='tw-font-500 tw-text-24'>Notify Participants</label>
