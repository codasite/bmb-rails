...
export const PairedBracket = (props: PairedBracketProps) => {
	...
	const buildMatches = (rounds: Round[]) => {
		...
		const leftMatchColumns = getMatchColumns(leftMatches, 'left')
		const rightMatchColumns = getMatchColumns(rightMatches, 'right')
		const finalMatchColumn = getMatchColumns([finalMatch], 'center')
		...
		<>
			<div className={`wpbb-bracket wpbb-paired-bracket wpbb-${numRounds}-rounds${darkMode ? ' wpbb-dark-mode' : ''}`}>
				<div className='wpbb-bracket-rounds-container'>
					...
					{rounds.length > 0 && buildMatches(rounds)}
				</div>
