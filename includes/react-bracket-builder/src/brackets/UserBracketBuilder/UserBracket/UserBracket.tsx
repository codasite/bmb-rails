...
import { ShuffleTeam } from '../../UserTemplateBuilder/ShuffleTeam';
import UserTemplateBuilder, { BracketFields } from '../../UserTemplateBuilder/UserTemplateBuilder';
import '../../UserTemplateBuilder/user-template-builder.scss'
...
const UserBracket = (props: UserBracketProps) => {
	...
	const renderPairedBracket = (bracketProps) => {
		...
		const handleShuffle = () => {
			...
			// setMatchTree(matchValue)
			dispatch(setMatchTree(matchValue))
		}
		const handleSave = () => {
			setIsLoading(true)
		...
		return (
			<div>
				{isLoading ? <Loader /> :
					<div>
						{prevComponent ? <UserTemplateBuilder apparelUrl={apparelUrl} bracketStylesheetUrl={bracketStylesheetUrl}></UserTemplateBuilder> :
							<div className={`wpbb-bracket-container wpbb-${numRounds}-rounds${darkMode ? ' wpbb-dark-mode' : ''}`}>
								<div>
									<div>{canEdit && <Button className='create-bracket' onClick={handleRedirect}><ArrowNarrowLeft />CREATE BRACKET</Button>}</div>
									{matchTree ? [
										<div>
											<div>{canPick && < ThemeSelector darkMode={darkMode} setDarkMode={setDarkMode} />}</div>
											<div className={'wpbb-slogan-container' + (pickedWinner ? ' invisible' : ' visible')}>
												<span className={'wpbb-slogan-text'}>{canEdit ? bracketFields?.bracketTitle : 'WHO YOU GOT?'}</span>
											</div>
											...
											<div>
												{canPick ?
													...
													<div>
														...
														<div className='bracket-button action-buttons'>
