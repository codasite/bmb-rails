...
const Options = () => {
    const [firstNum, setFirstNum] = useState(16);
    const [secondNum, setSecondNum] = useState(32);
    const [thirdNum, setThirdNum] = useState(64);
    const [selectedBox, setSelectedBox] = useState(firstNum);
   
    const handleBoxClick = (currentValue) => {
        console.log("BoxNum is:" + currentValue);
        setSelectedBox(currentValue);
        console.log("Selected is:" + selectedBox)
    };
    ...
    const performOperation = (op) => {
        console.log("Operation inside performOperation is:" + op);
        console.log("Selected box inside performOperation is:" + selectedBox);
        ...
        <div className={'options-page'}>
            <div className={'options-bracket-display'}>
                <div className={'randomize-teams'}>
                    ...
                    </svg><span className={'randomize-teams-text'}>Randomize Teams</span>
                </div>
                ...
                <Container>
                    <Row className="justify-content-center">
                        <div className="d-flex g-10">
                            <div
                                className={`custom-col custom-col-2 ${selectedBox === firstNum  ? 'highlight' : ''}`}
                                onClick={() => handleBoxClick(firstNum)}
                            >
                                {firstNum}
                            </div>
                            <div
                                className={`custom-col custom-col-2 ${selectedBox === secondNum  ? 'highlight' : ''}`}
                                onClick={() => handleBoxClick(secondNum)}
                            >
                                {secondNum}
                            </div>
                            <div
                                className={`custom-col custom-col-2 ${selectedBox === thirdNum  ? 'highlight' : ''}`}
                                onClick={() => handleBoxClick(thirdNum)}
                            >
                                {thirdNum}
                            </div>
                        </div>
