...
export const MyTemplatesModal = (props: MyTemplatesModalProps) => {
  ...
  useEffect(() => {
    const buttons = document.getElementsByClassName('wpbb-host-tournament-button');
    if (buttons.length === 0) {
      return
    }
  ...
  return (
    <Modal show={showModal} setShow={setShowModal}>
      <div className='tw-flex tw-flex-col'>
        <h1 className="tw-text-32 tw-leading-10 tw-font-white tw-whitespace-pre-line tw-mb-30">{editFormHeader}</h1>
        <input
          className={`tw-placeholder-${hasError ? 'red/60' : 'white/60'} tw-border-0 tw-border-b tw-border-white tw-mb-30 tw-border-solid tw-p-15 tw-outline-none tw-bg-transparent tw-text-16 tw-text-white tw-font-sans tw-w-full` +
