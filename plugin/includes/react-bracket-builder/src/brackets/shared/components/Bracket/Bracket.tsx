...
const TeamSlot = (props: TeamSlotProps) => {
  ...
  const { team, updateTeam, pickTeam } = props
...
const MatchBox = (props: MatchBoxProps) => {
  const { match, direction, height, spacing, updateTeam, pickTeam } = props
  ...
  return (
    <div
      className={className}
      style={{ height: height, marginBottom: spacing }}
    >
      <TeamSlot
        className="wpbb-team1"
        team={match.team1}
        updateTeam={
          updateTeam ? (name: string) => updateTeam(true, name) : undefined
        }
        pickTeam={pickTeam ? () => pickTeam(true) : undefined}
      />
      {direction === Direction.Center && (
        <TeamSlot className="wpbb-champion-team" team={match.result} />
      )}
      <TeamSlot
        className="wpbb-team2"
        team={match.team2}
        updateTeam={
          updateTeam ? (name: string) => updateTeam(false, name) : undefined
        }
...
const RoundHeader = (props: RoundHeaderProps) => {
  ...
  const { round, updateRoundName } = props
...
const MatchColumn = (props: MatchColumnProps) => {
          ...
          updateTeam={
            canEdit
              ? (left: boolean, name: string) =>
                  updateTeam(round.id, matchIndex, left, name)
              : undefined
          }
          pickTeam={
            pickTeam ? (left: boolean) => pickTeam(matchIndex, left) : undefined
          }
        />
      )
    })
  ...
  return (
    <div className="wpbb-round">
      ...
      />
      <div className="wpbb-round__body">{buildMatches()}</div>
    </div>
  )
}
...
export const Bracket = (props: BracketProps) => {
  ...
  const updateRoundName = (roundId: number, name: string) => {
    ...
    const roundToUpdate = newMatchTree.rounds.find(
  ...
  const buildRounds2 = (rounds: Round[]) => {
    ...
    return [
        ...
        .map((round, idx) => {
          ...
          return (
            <MatchColumn
              ...
              pickTeam={
                canPick
                  ? (matchIndex: number, left: boolean) =>
                      pickTeam(round.depth, matchIndex, left)
                  : undefined
              }
            />
          )
      ...
      <MatchColumn
        ...
        updateTeam={canEdit ? updateTeam : undefined}
        pickTeam={
          canPick ? (_, left: boolean) => pickTeam(0, 0, left) : undefined
        }
      />,
...
function getElementChildrenAndStyles(element: HTMLElement): string {
  ...
  const style = rulesUsed
    .map((cssRule) => {
      ...
      } { ${cssRule.style.cssText.toLowerCase()} }`
    })
    .join('\n')
    .replace(/(\{|;)\s+/g, '$1\n  ')
    .replace(/\A\s+}/, '}')
