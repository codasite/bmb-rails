...
const NumTeamsIncrementButton = (props: NumTeamsIncrementButtonProps) => {
  const { onPressed, children, active } = props
  const baseStyles = [
    'tw-flex',
    'tw-justify-center',
    'tw-items-center',
  ...
  const activeStyles = ['tw-border-white', 'tw-text-white', 'tw-cursor-pointer']
  const inactiveStyles = ['tw-border-white/50', 'tw-text-white/50']
...
export const NumTeamsPicker = (props: NumTeamsPickerProps) => {
  ...
  const inactiveStyles = ['tw-border', 'tw-border-white/50']
  const activeStyles = ['tw-bg-green/15', 'tw-border-4', 'tw-border-green']

  const styles = baseStyles
    .concat(selected ? activeStyles : inactiveStyles)
    .join(' ')

  return (
    <div className={'tw-flex tw-flex-col tw-gap-24 tw-grow'}>
      <div className={styles} onClick={handleBoxClick}>
        <span className="tw-font-500 tw-text-48 tw-text-white">
          {currentValue}
        </span>
        {selected && currentValue === defaultValue && (
          <span className="tw-absolute tw-bottom-8 tw-left-14 tw-text-green tw-font-500 tw-text-12">
            Default
          </span>
        )}
      </div>
      {selected && (
        <div className="tw-flex tw-justify-center tw-gap-12">
          <NumTeamsIncrementButton
            active={!decrementDisabled}
            onPressed={handleDecrement}
          >
            -
          </NumTeamsIncrementButton>
          <NumTeamsIncrementButton
            active={!incrementDisabled}
