...
const NumRoundsSelector = (props) => {
  const { numRounds, setNumRounds } = props

  const minRounds = 1
  const maxRounds = 6

  const options = Array.from(Array(maxRounds - minRounds + 1).keys()).map(
    (i) => {
      return <option value={i + minRounds}>{i + minRounds}</option>
    }
  )
...
const NumWildcardsSelector = (props) => {
  const { numWildcards, setNumWildcards, maxWildcards } = props
  ...
  options = [
    ...options,
    ...Array.from(Array(maxWildcards / 2).keys())
      .reverse()
      .map((i) => {
        return <option value={(i + 1) * 2}>{(i + 1) * 2}</option>
      }),
  ]
...
const WildcardPlacementSelector = (props: WildcardPlacementSelectorProps) => {
  const { wildcardPlacement, setWildcardPlacement, disabled } = props
  ...
  return (
    <div className="wpbb-option-group">
      <label>Wildcard Placement:</label>
      <select
  ...
  return (
    ...
    >
      ...
      <Modal.Body className="pt-0 wpbb-default">
        {matchTree ? (
          <Bracket matchTree={matchTree} setMatchTree={setMatchTree} canPick />
        ) : (
          'Loading...'
        )}
      </Modal.Body>
...
const NewBracketModal = (props: NewBracketModalProps) => {
  ...
  const { show, handleClose, handleSave, bracketId } = props
  ...
  const rebuildMatchTree = (
    ...
    setMatchTree(
      MatchTree.fromOptions(
        updatedNumRounds,
        updatedNumWildcards,
        updatedWildcardPlacement
      )
    )
  }

  const handleSaveBracket = () => {
    const req = matchTree.toRequest(
      bracketName,
      true,
      numRounds,
      numWildcards,
    ...
    bracketApi.createBracket(req).then((newBracket) => {
  ...
  return (
    ...
    >
      <Modal.Header className="wpbb-bracket-modal__header" closeButton>
        <Modal.Title>
          <BracketTitle title={bracketName} setTitle={setBracketName} />
        </Modal.Title>
