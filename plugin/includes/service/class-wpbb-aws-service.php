...
class Wpbb_Lambda_Service {
  ...
  private function image_from_api($body): array|WP_Error {
    ...
    $res = wp_remote_post($convert_url, [
    ...
    if (is_wp_error($res) || wp_remote_retrieve_response_code($res) !== 200) {
      return new WP_Error(
        'error',
        __('There was an error converting the html to an image', 'text-domain'),
        ['status' => 500]
      );
    }
  ...
  private function image_from_invocation($params): array|WP_Error {
    if (!defined('HTML_TO_IMAGE_FUNCTION_NAME')) {
      return new WP_Error(
        'error',
        __('Lambda function name not defined.', 'text-domain'),
        ['status' => 500]
      );
    }
    ...
    if (!isset($params['s3Bucket'])) {
      if (!defined('BRACKET_BUILDER_S3_IMAGE_BUCKET')) {
        return new WP_Error(
          'error',
          __('S3 image bucket name not found.', 'text-domain'),
          ['status' => 500]
        );
      }
    ...
    if (!isset($params['s3Key'])) {
      $extension =
        isset($params['pdf']) && $params['pdf'] === true ? '.pdf' : '.png';
  ...
  private function invoke($functionName, $params, $logType = 'None') {
    ...
    if (!isset($this->lambdaClient)) {
      return new WP_Error(
        'error',
        __(
          'Lambda client is not initialized. Did you forget to add AWS credentials?',
          'text-domain'
        ),
        ['status' => 500]
      );
