<?php
require_once plugin_dir_path(dirname(__FILE__)) . '/domain/class-wpbb-bracket.php';
...
class Wpbb_ApiValidation
{
    ...
    private function check_team_names()
    {
        ...
        foreach ($round->matches as $index => $match) {
            if (is_null($match) && $this->wildcards != 0) {
                ...
                if (is_null($team)) {
                    return $this->createError('Wild Card Team Name not found in second round');
                }
                $teams_found++;
            } elseif (is_null($match)|| is_null($match->team1) || is_null($match->team2)) {
    ...
    public function check_inner_round_teams()
    {
        ...
        if ($this->wildcards != 0) {
            ...
            foreach ($round->matches as $index => $match) {
                if (!is_null($match)) {
                    $team_name = $second_round->matches[floor($index / 2)]->{"team" . ($index % 2 === 0 ? 1 : 2)};
        ...
        $no_of_rounds_to_check =  $this->total_rounds - ($this->wildcards != 0 ? 2 : 1);
 
        foreach ($this->bracket->rounds as $round) {
            if ($round->depth < $no_of_rounds_to_check) {
                foreach ($round->matches as $match) {
                    if (!is_null($match->team1) || !is_null($match->team2)) {
                        return $this->createError('Team Name should not accept in ' . $round->name);
                    }
                }
            }
        }
    ...
    public function is_valid_wildcard()
    {
        ...
        return ($this->wildcards >= 0 && $this->wildcards <= ($total_teams / 2) && ($this->wildcards % 2 === 0));
    }

    private function createError($message)
    {
        return new WP_Error('cant-create', __($message, 'text-domain'), array('status' => 400));
    }
    ...
    public function get_team_count($rounds, $wildcards) {
        ...
        if ($wildcards != 0) {
            ...
            $total_no_of_teams = $teams_in_first_round + (($total_no_of_teams - $teams_in_first_round)/2);
        }
