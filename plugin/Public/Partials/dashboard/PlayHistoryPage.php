...
class PlayHistoryPage {
  private PlayRepo $play_repo;
  public function __construct($args = []) {
    $this->play_repo = $args['play_repo'] ?? new PlayRepo();
  }

  static function view_bust_btn($endpoint): false|string {
    return PartialsCommon::view_play_btn($endpoint, 'View Bust');
  }

  static function bust_again_btn($endpoint): false|string {
    ob_start(); ?>
    <a class="tw-border-red tw-border-solid tw-border tw-bg-red/15 hover:tw-bg-red hover:tw-text-dd-blue tw-px-16 tw-py-12 tw-flex tw-justify-center tw-rounded-8 tw-text-white" href="<?php echo esc_url(
      $endpoint
    ); ?>">
  ...
  static function get_default_play_buttons($play): false|string {
  ...
  static function get_buster_play_buttons($play): false|string {
    $view_link = get_permalink($play->id) . 'view';
    $bust_again_link = get_permalink($play->busted_id) . 'bust';
    ob_start();
    ?>
		<?php echo self::view_bust_btn($view_link); ?>
		<?php echo self::bust_again_btn($bust_again_link); ?>
	<?php return ob_get_clean();
  }
