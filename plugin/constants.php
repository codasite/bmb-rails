<?php
// Stub file. Do not include this file anywhere to avoid overriding the globals defined in wp-config.php
const AWS_ACCESS_KEY_ID = '';
const AWS_SECRET_ACCESS_KEY = '';
const HTML_TO_IMAGE_FUNCTION_NAME = '';
const WP_SENTRY_ENV = '';
const WP_SENTRY_PHP_DSN = '';
const WPBB_S3_IMAGE_BUCKET = '';
const WPBB_S3_ORDER_BUCKET = '';
const BRACKET_PRODUCT_CATEGORY = '';
const BRACKET_PRODUCT_SHOP_SLUG = '';
const BRACKET_PLACEMENT_CENTER_CAT = '';
const WPBB_SERVICE_USER = '';
const MAILCHIMP_API_KEY = '';
const MAILCHIMP_FROM_EMAIL = '';
const IMAGE_GENERATOR_PATH = '';
const BMB_PLUS_SLUG = '';
const WPBB_DB_PREFIX = '';
const HOUR_IN_SECONDS = 3600;
const DAY_IN_SECONDS = 86400; // 24 * HOUR_IN_SECONDS
const COOKIEPATH = '';
// Bypass all image generator calls and s3 calls so we can run locally
// @var string|null
const DISABLE_IMAGE_GENERATOR_CALLS = '';
const WPBB_PLUGIN_DIR = '';
const FIREBASE_CREDENTIALS_PATH = 'path/to/firebase/credentials.json';
const ABSPATH = '';
const ARRAY_A = 'ARRAY_A';
const COOKIE_DOMAIN = '';
const STRIPE_SECRET_KEY = '';
const STRIPE_WEBHOOK_SECRET = '';
const STRIPE_PUBLISHABLE_KEY = '';
const EP_PAGES = 4096;
const EP_ROOT = 1;
