...
class RoundCompleteMessageFormatterTest extends TestCase {
  ...
  public function test_get_button_url_suffix_should_return_results_when_bracket_is_complete() {
    $bracket = new Bracket(['status' => 'complete']);
    $suffix = RoundCompleteMessageFormatter::get_button_url_suffix($bracket);
    $this->assertEquals('results', $suffix);
  }

  public function test_get_button_url_suffix_should_return_play_when_bracket_is_not_complete() {
    $bracket = new Bracket(['status' => 'live']);
    $suffix = RoundCompleteMessageFormatter::get_button_url_suffix($bracket);
    $this->assertEquals('play', $suffix);
  }
}
