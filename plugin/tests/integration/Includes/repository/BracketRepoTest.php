...
class BracketRepoTest extends WPBB_UnitTestCase {
  ...
  public function test_new_results_sets_updated_at() {
    $bracket = $this->create_bracket([
      'status' => 'publish',
      'num_teams' => 4,
    ]);
    ...
    $results = $this->get_bracket($bracket->id)->results;
    $this->assertNotNull($results[0]->get_updated_at());
  }
  ...
  public function test_update_results_with_winning_team_pick_percent() {
    ...
    $bracket = $this->bracket_repo->update($bracket->id, [
      'results' => [
        [
          ...
          'match_index' => 0,
          'winning_team_id' => $winning_team_id,
          'winning_team_pick_percent' => 0.75,
        ],
      ],
    ]);
    ...
    $this->assertEquals(0.75, $new_results[0]->winning_team_pick_percent);
  }
}
