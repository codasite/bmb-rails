...
class GelatoPublicHooks {
  ...
  public function __construct(GelatoProductIntegration $gelato, $opts = []) {
    $this->gelato = $gelato;
    $this->bracket_product_utils =
      $opts['bracket_product_utils'] ?? new BracketProductUtils();
    $this->utils = $opts['utils'] ?? new Utils();
    $this->s3 = $opts['s3'] ?? new S3Service();
    $this->pdf_service = $opts['pdf_service'] ?? new PdfService();
